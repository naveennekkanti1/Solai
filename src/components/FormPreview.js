...
const FormPreview = () => {
   ...
   * Fetch form data
   *
   * useCallback is used because this function is referenced
  ...
  const fetchFormData = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `https://violent-stacey-solai-aba6a507.koyeb.app/v1/forms/${encodeURIComponent(
          formId
        )}`
      );

      console.log("Form data received:", res.data);
      console.log("Fields received:", res.data.fields);
  ...
  /*
   * Fetch form whenever formId changes.
   *
   * fetchFormData is now included in the dependency array,
   * which resolves react-hooks/exhaustive-deps.
   */
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://violent-stacey-solai-aba6a507.koyeb.app/v1/forms/submit",
        payload
      );
  ...
  const getFieldTypeDisplay = (fieldType, field) => {
    ...
    // If fieldType is null but options exist,
    // assume it is an OPTIONS field.
          ...
          {isDeadlinePassed && (
            <div className="mt-4 p-4 bg-red-50 border-2 border-red-200 rounded-xl">
              <p className="text-red-700 font-semibold text-center">
                This form has passed its deadline and is no longer
                accepting responses.
              </p>
            </div>
          )}
