...
const EditForm = () => {
  ...
  useEffect(() => {
    fetch(
      `https://violent-stacey-solai-aba6a507.koyeb.app/v1/forms/${id}`
    )
      .then((res) => res.json())
      .then((data) => {
        ...
        if (form.formsettings) {
          ...
          setAllowMultipleResponses(
            form.formsettings.allowedMultipleResponses || false
          );
          setEditAfterSubmit(
            form.formsettings.editAfterSubmit || false
          );
        }
  ...
  const handleUpdate = async (e) => {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      if (
        fields[i].fieldType === "OPTIONS" &&
        fields[i].options.length === 0
      ) {
        alert(
          `Please add options for field "${fields[i].label}"`
        );
        return;
      }
    }
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border-2 border-gray-200 md:col-span-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-600">
                          Users can modify their responses
                        </p>
                      </div>
                    </div>
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          setEditAfterSubmit(e.target.checked)
                        }
                        className="sr-only peer"
                      />
            ...
            {fields.length === 0 ? (
              <div className="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
                ...
                <p className="text-gray-500 text-sm">
                  Click "Add Field" to create your first question
                </p>
              </div>
            ) : (
              <div className="space-y-4">
